function [sys,x0,str,tss]=RBsuperheater1(t,x,u,flag,Param4,X_ss)
...
%properties 
h_boilRB=XSteam('hV_p',RB_p); %enthalphy for saturated steam kJ/kg
Ts_in= XSteam ('T_ph',RB_p,h_boilRB);%Steam inlet temperature (saturated)
rho_satRB_in= XSteam ('rhoV_p',RB_p); %density sat_steam (in) from boiler 
rho_SHRB_out= XSteam ('rho_pT',RB_p, Ts_out1); %density steam (out) 
Cp_s_in = XSteam ('Cp_pT',RB_p,Ts_in) ; %Cp steam T_in
Cp_s_out = XSteam ('Cp_pT',RB_p,Ts_out1) ; %Cp steam T_out
...
rho_s= (rho_satRB_in+rho_SHRB_out)/2; %density steam average
...
rho_satRB_in= XSteam ('rhoV_p',RB_p); %density sat_steam (in) from boiler 
rho_SHRB_out= XSteam ('rho_pT',RB_p, Ts_out1); %density steam (out) 
Cp_s_in = XSteam ('Cp_pT',RB_p,Ts_in);  %Cp steam T_in kJ/(kg oC)
Cp_s_out = XSteam ('Cp_pT',RB_p,Ts_out1); %Cp steam T_out kJ/(kg oC)
...
sys(1) =(steam_flow*(Cp_s* Ts_in - Cp_s*Ts_out1) + U*(2*pi*r_tube*L_tube*n_tube)*(T_FG_out1-Ts_out1))/(rho_s*Cp_s*(pi*r_tube^2*L_tube*n_tube)+M_tube*Cp_M);
