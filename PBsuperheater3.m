function [sys,x0,str,tss]=PBsuperheater3(t,x,u,flag,Param3,X_ss)
...
%properties 
rho_SHPB_in= XSteam ('rho_pT',PB_p,Ts_in3); %density steam (in) from previous compartment
rho_SHPB_out= XSteam ('rho_pT',PB_p, Ts_out3); %density steam (out) 
Cp_s_in = XSteam ('Cp_pT',PB_p,Ts_in3) ; %Cp steam T_in
Cp_s_out = XSteam ('Cp_pT',PB_p,Ts_out3) ; %Cp steam T_out
...
sys(1) =(steam_flow *Cp_s *(Ts_in3-Ts_out3) + U*(2*pi*r_tube*L_tube*n_tube)*(T_FG_out3-Ts_out3))/(rho_s*(pi*r_tube^2*L_tube*n_tube)*Cp_s+M_tube*Cp_M);
