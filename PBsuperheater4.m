function [sys,x0,str,tss]=PBsuperheater4(t,x,u,flag,Param3,X_ss)
...
%properties 
rho_SHPB_in= XSteam ('rho_pT',PB_p,Ts_in4); %density steam (in) from previous compartment
rho_SHPB_out= XSteam ('rho_pT',PB_p, Ts_out4); %density steam (out) 
Cp_s_in = XSteam ('Cp_pT',PB_p,Ts_in4) ; %Cp steam T_in
Cp_s_out = XSteam ('Cp_pT',PB_p,Ts_out4) ; %Cp steam T_out
...
sys(1) =(steam_flow *Cp_s *(Ts_in4-Ts_out4) + U*(2*pi*r_tube*L_tube*n_tube)*(T_FG_out4-Ts_out4))/(rho_s*(pi*r_tube^2*L_tube*n_tube)*Cp_s+M_tube*Cp_M);
