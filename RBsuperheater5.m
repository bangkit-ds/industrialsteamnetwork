function [sys,x0,str,tss]=RBsuperheater5(t,x,u,flag,Param4,X_ss)
...
%properties 
rho_SHRB_in= XSteam ('rho_pT',RB_p,Ts_in5); %density steam (in) from previous compartment
rho_SHRB_out= XSteam ('rho_pT',RB_p, Ts_out5); %density steam (out) 
Cp_s_in = XSteam ('Cp_pT',RB_p,Ts_in5) ; %Cp steam T_in
Cp_s_out = XSteam ('Cp_pT',RB_p,Ts_out5) ; %Cp steam T_out
...
rho_SHRB_out= XSteam ('rho_pT',RB_p, Ts_out5); %density steam (out) 
Cp_s_in = XSteam ('Cp_pT',RB_p,Ts_in5) ; %Cp steam T_in
Cp_s_out = XSteam ('Cp_pT',RB_p,Ts_out5) ; %Cp steam T_ou
...
sys(1) =(steam_flow *Cp_s *(Ts_in5-Ts_out5) + U*(2*pi*r_tube*L_tube*n_tube)*(T_FG_out5-Ts_out5))/(rho_s*Cp_s*(pi*r_tube^2*L_tube*n_tube)+M_tube*Cp_M);
