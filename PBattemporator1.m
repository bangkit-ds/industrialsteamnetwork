function [sys,x0,str,tss]=PBattemporator1(t,x,u,flag,Param3,X_ss)
...
%properties 
rho_SHPB_in= XSteam ('rho_pT',PB_p,Ts_in_attemp1); %density steam (in) from previous compartment
rho_SHPB_out= XSteam ('rho_pT',PB_p, Ts_out_attemp1); %density steam (out) 
Cp_s_in = XSteam ('Cp_pT',PB_p,Ts_in_attemp1) ; %Cp steam T_in
Cp_s_out = XSteam ('Cp_pT',PB_p,Ts_out_attemp1) ; %Cp steam T_out
...
%sys (1)= ((steam_flow*Cp_s*Ts_in_attemp1-steam_flow*Cp_s*Ts_out_attemp1) + water_attemp1_flow*(Cp_attemp_in*T_water-Cp_s*Ts_out_attemp1))/(rho_s*(pi*r_attempt1^2*L_attemp1)*Cp_s+M_attemporator*Cp_M);
